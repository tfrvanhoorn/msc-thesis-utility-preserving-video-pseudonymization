def sequences(all_path, all_feature, win = 10, step = 5):  
    ...
    for id_cur in range(0, len(all_path)+1, step):
        ...
        curr_FE_AN = all_feature[id_cur:need_id]
        if hasattr(curr_FE_AN, "detach"):
            curr_FE_AN = curr_FE_AN.detach().cpu().numpy()
        elif hasattr(curr_FE_AN, "numpy"):
            curr_FE_AN = curr_FE_AN.numpy()
        curr_FE_AN = curr_FE_AN.tolist()
